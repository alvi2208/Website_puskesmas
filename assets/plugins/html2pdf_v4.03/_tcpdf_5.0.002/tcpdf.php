	...
	class TCPDF {
		...
		public function set_mqr($mqr) {
			if(!defined('PHP_VERSION_ID')) {
				$version = PHP_VERSION;
				define('PHP_VERSION_ID', (($version[0] * 10000) + ($version[3] * 100) + $version[4]));
			}
		 ...
		 * @since 4.4.000 (2008-12-07)
		 */protected
		 function _putfontwidths(&$font, &$cidoffset)

		 protected function _putfontwidths($font, $cidoffset = 0) {
			if (!isset($font['cw'])) {
				// Handle the case where 'cw' is not set
				return; // or perform any other error handling
			}
		
		

    // Sorting the character widths array
    ksort($font['cw']);
			$rangeid = 0;
			$range = array();
			$prevcid = -2;
